...
public class DeadClass implements ast.Visitor {
	...
	private java.util.HashMap<String, String> newClassExtends; 
	...
	public void visit(ast.classs.Class c) {
		if (null != c.extendss) {
			String extendss = c.extendss;
			while (null != extendss) {
				if (this.set.contains(extendss))
					;
				else {
					this.worklist.add(extendss);
					this.set.add(extendss);
				}
				extendss = this.newClassExtends.get(extendss);
			}
		}
	...
	public void visit(ast.program.Program p) {
		...
		this.newClassExtends = new java.util.HashMap<String, String>();
		for (ast.classs.T c : p.classes) {
			ast.classs.Class current = (ast.classs.Class) c;
			if (null != current.extendss) {
				this.newClassExtends.put(current.id, current.extendss);
			}
		}

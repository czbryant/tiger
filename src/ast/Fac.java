...
public class Fac {
	...
	static ast.mainClass.MainClass factorial = new ast.mainClass.MainClass(
			...
			new Flist<ast.stm.T>().addAll(new ast.stm.Print(new ast.exp.Call(
							...
							new util.Flist<ast.stm.T>().addAll(
									new ast.stm.If(
										new ast.exp.Lt(new ast.exp.Id("num"),
												new ast.exp.Num(1)),
										new ast.stm.Block(new Flist<ast.stm.T>().addAll( new ast.stm.Assign("num_aux",
												new ast.exp.Num(1)))),
										new ast.stm.Block(new Flist<ast.stm.T>().addAll(new ast.stm.Assign(
												"num_aux",
												new ast.exp.Times(
														new ast.exp.Id("num"),
														new ast.exp.Call(
	...
	// // class "Doit"
		static ast.classs.Class doit = new ast.classs.Class(
				"Doit",
				null,
				new util.Flist<ast.dec.T>().addAll(),
	...
	// // main class: "Factorial"
	static ast.mainClass.MainClass mainSum = new ast.mainClass.MainClass("Sum",
			"a",
			new util.Flist<ast.dec.T>().addAll(new ast.dec.Dec(
					new ast.type.Int(), "i")),
			new Flist<ast.stm.T>().addAll(new ast.stm.Print(new ast.exp.Call(new ast.exp.NewObject(
					"Doit"), "doit",
	...
	// program
	public static ast.program.Program progSum = new ast.program.Program(
			mainSum, new util.Flist<ast.classs.T>().addAll(doit));
}
